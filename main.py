...
#   지금까지 같은 것을 네 곳에서 다른 숫자로 쟀다 —
#     BASE_PERSONA「발화 스타일(공통)」 2~3문장 · 감정 1~2문장 ·
#     상황극 진행 규칙 1~2문장 · LEVEL_RULES 초급 한 문장.
#   ★ 눈금이 여럿이면 **큰 쪽이 이긴다.** 「공통」이라 적힌 2~3문장이 제일 셌다.
#   그래서 v161에서 등급을 2로 내려도 어휘·문법만 쉬워지고 **길이는 그대로**였다.
#   한마디를 겨우 만드는 학습자에게는 길이가 어휘보다 더 큰 벽이다.
#   → 아래 한 값에서만 재고, 다른 자리는 「[한 턴 길이]」라고 이름으로만 가리킨다.
LV_TURN = "한 문장 + 짧은 물음 하나" if MAX_LEVEL <= 2 else "2~3문장 이내"
TURN_LEN_RULE = (
"""[한 턴 길이] — 음성 대화다. 네 차례는 이만큼이다.
- 한 턴에 **한 문장**이다. 거기에 짧은 물음 하나까지만 더 붙일 수 있다.
  (○) "아 진짜? 뭐 먹었어?"
  (✕) "아 진짜. 나도 어제 김치찌개 먹었는데 진짜 맛있더라. 뭐 먹었어?"
  ★ 위 보기는 **길이만** 보여 준다. 말투는 화계 규칙을 그대로 따라라.
- ★ 두 번째 문장을 덧붙이고 싶으면 참아라. 그 자리가 학습자가 말할 자리다.
- 설명·감상·덧붙임은 학습자가 물어보면 그때 한다. 먼저 내놓지 마라."""
    if MAX_LEVEL <= 2 else
"""[한 턴 길이] — 음성 대화다. 네 차례는 이만큼이다.
- 한 턴에 2~3문장 이내로 짧게."""
)
...
- 한 턴에 몇 문장인지는 [한 턴 길이]에서 정한다. 여기서 다시 정하지 않는다.
"""
...
- ★ 위 ①②③은 [한 턴 길이]의 **하나뿐인 예외**다. 그 밖에는 예외가 없다.
{"  다만 초급이면 ①을 접고 ②③만 한 번에 붙여라 — 「'학교에서 공부했어.' 다시 말해 볼래?」" if MAX_LEVEL <= 2 else ""}
