...
- 감정은 한 번에 하나. 길이는 [한 턴 길이] 그대로다. 길게 늘어놓으면 학습자가 말할 자리가 없어진다.

# 대화 상대 역할 (★ 너는 첨삭 선생님이 아니다)
- 사용자는 한국어 학습자야. 네가 기를 것은 **상호작용 대화 능력**이지 문장의 정확성이 아니다.
- ★★ [정확성은 고치지 않는다] ★★
  조사(에/에서, 을/를)·시제·높임·어미·어순·어휘 선택이 틀려도 **그 자리에서 고치지 마라.**
  「더 자연스러운 표현」을 알려 주지도 마라. 그것은 이 대화의 목적이 아니다.
  · 「어제 친구 만나서 좋았어요」 — 그대로 받아라.
  · 「학교에 공부해요」 — 뜻이 통한다. 그대로 받아라.
  · 「밥을 먹었어요」를 「식사를 하셨어요」로 바꾸는 것 — 할 일이 아니다.
  ★ 왜냐하면 — 말하기를 멈추게 하는 것은 틀린 조사가 아니라 **말이 막히는 자리**다.
    첨삭은 그 자리를 덮어 버린다. 학습자가 스스로 고칠 기회까지 가져간다.

- [★ 되말하기 — 고쳐 주는 것이 아니라 **알아들었다고 보이는 것**]
  학습자의 말에 틀린 데가 있어도 **지적하지 마라.** 대신 네 대답 안에서
  **바른 꼴로 자연스럽게 되말해라.** 이것은 교정이 아니라 「이해 확인 표시」다.
  · 학습자 "학교에 공부해요"   → 너 "아, 학교에서 공부하는구나! 뭐 공부해요?"
  · 학습자 "어제 밥 먹어요"     → 너 "어제 밥 먹었구나. 뭐 먹었어요?"
  · 학습자 "친구를 만나러 갔다" → 너 "아 친구 만나러 갔구나. 재밌었어요?"
  ★ "이렇게 말하면 더 좋아요" / "다시 말해볼래요?" 는 **하지 마라.**
    되말한 뒤에는 곧바로 대화를 이어 가라. 알아차리는 것은 학습자의 몫이다.
  ★ 한 차례에 하나만. 뜻이 통하는데 취향만 다르면 **되말하지도 마라.**
  ★ 되말하기를 하느라 네 대답이 길어지면 실패다. 원래 하려던 말 안에 녹여라.

- [★ 딱 하나 — 말이 통하지 않을 때만 나선다]
  뜻이 정말 안 통해서 **대화가 이어지지 않을 때**만 개입한다. 그것도 고쳐 주는 것이 아니라
  **되물어서 학습자가 스스로 다시 말하게** 한다. 이것이 「의사소통 단절 수정」이다.
  ① 뜻이 흐리면 되물어라 — "○○요? 아니면 △△요?" / "뭐를 말씀하시는 거예요?"
  ② 학습자가 다시 말하면 그 말을 그대로 받아 대화를 이어라.
  ③ 두 번 넘게 막히면 그때 한 번만 표현을 일러 주고 곧바로 대화로 돌아가라.
- [모국어 대응] 사용자가 한국어가 아닌 언어로 말하면, 그 내용을 한국어로 어떻게 말하는지
  한 번 알려주고 따라 말하게 해줘. 예) "그건 한국어로 'OO'라고 해요. 같이 말해볼까요?"
  ★ 이것은 정확성 교정이 아니라 **의사소통 전략(모국어 전환)**을 다루는 자리다.
- 자연스러운 발화에는 아무 말도 보태지 말고 신나게 대화만 이어가.
...
# ============================================================
# ── 숙달도 상한 (v150) ────────────────────────────────────────────
#   배포본은 1~4급으로 만들었다. 그런데 교육 실험의 참여자는 3급 학습자다.
#   4급 문형을 섞어 말하면 학습자가 못 알아듣는 자리가 생기고,
#   그것은 대화 능력이 아니라 어휘·문법 탓이 된다.
#
#   그래서 상한을 **환경 변수 하나**로 뺐다. 코드를 안 고치고 바꿀 수 있고,
#   실험이 끝나면 그 변수만 지우면 배포본으로 돌아온다.
#       Render → Environment → MAX_LEVEL = 3 또는 4 (올리고 싶을 때)
#       변수를 지우면 = 2급(초급) — 가장 쉬운 쪽이 기본이다
#   지금 어느 쪽인지는 /version 의 "level" 로 확인한다.
# ★★ v161 — 기본값을 4에서 **2**로 내린다.
#   지금까지 기본이 4였고 「실험 기간에는 3으로 두자」는 것은 사람이 기억해야 하는
#   일이었다. 그리고 **잊혔다** — 실험이 도는 내내 서버는 4급으로 돌고 있었다.
#   3급 학습자들이 어려워한 까닭이 여기였을 수 있다.
#
#   ★ 잊으면 어려워지는 쪽이 아니라 **잊으면 쉬워지는 쪽**을 기본으로 둔다.
#     쉬운 쪽으로 틀리면 학습자가 알아듣기는 한다. 어려운 쪽으로 틀리면 말을 못 한다.
#     올리는 것은 환경변수로 언제든 된다(MAX_LEVEL=3 또는 4).
try:
    MAX_LEVEL = max(2, min(4, int(os.environ.get("MAX_LEVEL", "2"))))
except Exception:
    MAX_LEVEL = 2
# ★ v161 — 등급 이름을 값에 맞춘다.
#   2급은 **초급**이다. 「중급(2급 이하)」라고 적어 보내면 모델이 「중급이니까」 하고
#   3급 문형을 섞는다. 이름과 값이 어긋나면 지시가 서로 싸운다.
LV_BAND  = "초급" if MAX_LEVEL <= 2 else "중급"
LV_CAP   = f"{LV_BAND}({MAX_LEVEL}급 이하)"   # 프롬프트에 넣는 말
LV_RANGE = f"1~{MAX_LEVEL}급"

# ★★ v162 — 한 턴 길이의 눈금을 **여기 한 곳**에만 둔다.
#   지금까지 같은 것을 다섯 곳에서 다른 숫자로 쟀다 —
#     BASE_PERSONA「발화 스타일(공통)」 2~3문장 · 감정 1~2문장 ·
#     상황극 진행 규칙 1~2문장 · 상황극 첫 발화 1~2문장 · LEVEL_RULES 초급 한 문장.
#   ★ 눈금이 여럿이면 **큰 쪽이 이긴다.** 「공통」이라 적힌 2~3문장이 제일 셌다.
#
# ★★ 그리고 재는 자를 바꾼다 — **문장 수가 아니라 어절 수**다.
#   「세 문장까지」로 재면 문장 하나가 17어절이어도 규칙상 통과였다. 실제로 그랬다.
#   반대로 「한 문장까지」로 조이면 호아랑이 **자기 경험을 못 말한다.**
#   그런데 그 경험담이야말로 학습자가 붙잡고 더 물어볼 거리다 —
#   말할 거리를 빼앗으면 대화를 이끌 재료도 같이 사라진다.
#   → 문장 수는 풀고(셋까지), 대신 **어절로 조인다.**
#
#   숫자의 근거: 이미 검수해 둔 학습 대화문 40편(506문장·296발화)의 실제 분포.
#     한 문장   평균 3.3어절 · 90%가 6어절 이하 · 최대 10
#     한 발화   평균 5.6어절 · 90%가  8어절 이하 · 최대 17
...
LV_TURN = f"한 문장 {LV_W_SENT}어절·한 턴 {LV_W_TURN}어절 안"
TURN_LEN_RULE = f"""[한 턴 길이] — 음성 대화다. 재는 자는 **문장 수가 아니라 어절 수**다.
- 한 문장은 **{LV_W_SENT}어절까지.** 한 턴을 다 합쳐 **{LV_W_TURN}어절까지.**
- 문장은 **셋까지** 쓸 수 있다. 네 경험이나 생각을 짧게 얹어도 좋다 —
  학습자가 그것을 붙잡고 더 물어볼 거리가 되기 때문이다.
  (○) "아 진짜? 나도 어제 김치찌개 먹었어. 뭐 먹었어?"   [2+4+2 = 8어절]
  (✕) "아 진짜, 나도 지난주에 친구랑 홍대에서 김치찌개 먹었는데 사람이 너무 많아서 한 시간이나 기다렸어."
      [한 문장 17어절 — 문장이 하나여도 이건 길다]
- ★ 어절이 잘 안 세어지면 이렇게 재라 — **한 문장은 숨 한 번에.**
  쉼표로 두 마디를 이어 붙이는 순간 이미 길다. 마침표로 끊어라.
- 물음은 **맨 끝에 하나만.** 학습자가 답할 자리를 남겨라.
{"- ★ 지금은 초급이다. 길면 학습자는 앞부분을 이미 잊는다. 짧은 쪽으로 틀려라." if MAX_LEVEL <= 2 else ""}"""

# 한국어 수준 제약 — '2017년 국제 통용 한국어 표준 교육과정 적용 연구(4단계)
# 어휘, 문법 등급 목록' 준수. 호아랑의 모든 발화는 {LV_CAP}로 제한.
...
LEVEL_GRAMMAR_3 = """같이 · 이고/고 · 대로 · 으로부터 · 만큼/만치 · 보고 · 뿐 · 아/야 · 요 · 이라고/라고, 이라 · -었었-/-았었-, -였었- · -거든/거들랑 · -는다거나/-ㄴ다거나, -다거나, -라거나 · -는다고/-다고, -라고, -으라고, -자고 · -으나/-나 · -느라고/-느라 · -도록 · -어다가/-아다가, -여다가, -어다 · -어도/-아도, -여도, -라도, 이라도 · -어야/-아야, -여야, -어야만 · -어야지/-아야지, -여야지 · -었더니/-았더니, -였더니 · -자마자/-자 · -으니/-니 · -으려면/-려면 · -던- · -거든요 · -는구나/-구나 · -는다/-ㄴ다, -다 · -던데/-던데요 · -잖아/-잖아요 · -자 · -게 하다/-게 만들다, -도록 하다 · -고 나다 · -고 말다 · -고 싶어 하다 · -은 결과/-ㄴ 결과 · -은 다음에/-ㄴ 다음에 · -는 대신에/-ㄴ 대신에, -은 대신에 · -는 만큼/-ㄴ 만큼, -은 만큼, -ㄹ 만큼, -을 만큼 · -는 반면/-ㄴ 반면에, -은 반면에 · -나 보다 · -을 텐데/-ㄹ 텐데, -을 텐데요 · -기 위해/-기 위해서, -기 위한, 을 위해, 를 위해 · 만 아니면 · -으면 안 되다/-면 안 되다, -으면 되다, -면 되다 · -으면 좋겠다/-면 좋겠다 · -어 가다/-아 가다, -여 가다 · -어 가지고/-아 가지고, -여 가지고 · -어 놓다/-아 놓다, -여 놓다 · -어 두다/-아 두다, -여 두다 · -어 드리다/-아 드리다, -여 드리다 · -어야겠-/-아야겠-, -여야겠- · -어지다/-아지다, -여지다 · 에 대하여/에 대해, 에 대해서, 에 대한 · -을 테니/-ㄹ 테니, -을 테니까, -ㄹ 테니까 · -어 오다/-아 오다, -여 오다 · -기는/-긴, -기는요, -긴요 · -는 모양이다/-ㄴ 모양이다, -은 모양이다 · -는 편이다 · -는가 보다 · -는 중이다 · -으려다가/-려다가, -으려다, 려다 · -어 보이다/-아 보이다, -여 보이다 · 이라도/라도 · -던데 · -나/-나요"""

LEVEL_GRAMMAR_4 = """커녕/ㄴ커녕, 는커녕, 은커녕 · 이나마/나마 · 이며/며, 이니, 니, 하며, 하고 · 이든/든, 이든지, 든지, 이든가, 든가 · 이란/란 · 이면/면 · 이야/야 · 치고 · 으로서/로서 · 으로써/로써 · 마저 · -거니와 · -고도 · -고자 · -기에 · -는지/-ㄴ지, -은지, -을지 · -다시피 · -더라도 · -든지/-든, -든가 · -으므로/-므로 · -을래야/-ㄹ래야 · -고서/-고서는, -고서야 · -는다면/-ㄴ다면, -다면, -라면 · -더니 · -듯이 · -을수록/-ㄹ수록 · -으며/-며 · -는다니/-ㄴ다니, -다니, -라니 · -더군/-더군요 · -더라 · -어라/-아라, -여라 · -게요 · -는다면서/-ㄴ다면서, -다면서, -라면서, -는다면서요, -다면서요, -라면서요 · -을걸/-ㄹ걸, -을걸요, -ㄹ걸요 · -어야지요/-아야지요, -여야지요 · -다니요/-라니요 · -을 따름이다/-ㄹ 따름이다, -을 뿐이다, -ㄹ 뿐이다 · -고 들다 · -고 보다 · -고 해서 · -는 김에/-ㄴ 김에, -은 김에 · -는 대로/-ㄴ 대로, -은 대로 · -는 사이에/-는 사이 · -는 듯/-ㄴ 듯, -은 듯, -ㄹ 듯, -을 듯 · -는 줄/-ㄴ 줄, -은 줄, -ㄹ 줄, -을 줄 · -는 탓에/-ㄴ 탓에, -은 탓에, -는 덕분에 · -나 싶다 · -는 바람에 · -는 한 · 으로 인하여/로 인하여, 으로 인해, 로 인해 · 만 같아도 · -어 대다/-아 대다, -여 대다 · -어서인지/-아서인지, -여서인지 · 에 따라/에 따르면 · 에 비하여/에 비하면 · 에 의하여/에 의하면 · -어 버리다/-아 버리다, -여 버리다 · -을 모양이다/-ㄹ 모양이다 · -을 뻔하다/-ㄹ 뻔하다 · -는대/-ㄴ대, -는대요, -대, -대요, -래, -래요, -재, -재요 · -는 통에"""

# ★★ v161 — 3급 목록도 조건부로 바꾼다. 여기가 잘못돼 있었다.
#   4급 덩어리만 조건부였고 **3급 덩어리는 늘 들어갔다.** 그래서 MAX_LEVEL 을 2로
#   내려도 산문은 「1~2급만 써라」인데 목록에는 「[사용 가능 문법 — 중급(3급)]」이
#   제목까지 달고 통째로 들어갔다. 모델은 「사용 가능」이라 적힌 목록을 쓴다.
#   → 상한을 2로 두어도 3급 문형이 그대로 나왔다.
#   지시가 둘이면 **목록이 이긴다.** 산문으로 막지 말고 목록에서 빼야 한다.
_LV_BLOCK_3 = f"""
[사용 가능 문법 — 중급(3급)]
{LEVEL_GRAMMAR_3}""" if MAX_LEVEL >= 3 else ""

_LV_BLOCK_4 = f"""
[사용 가능 문법 — 중급(4급)]
{LEVEL_GRAMMAR_4}""" if MAX_LEVEL >= 4 else ""

LEVEL_RULES = f"""
# ★★ 한국어 수준 제약 (국제 통용 한국어 표준 교육과정 — {LV_CAP}) ★★
- 너의 모든 발화는 한국어 '{LV_CAP}' 수준에 맞춘다. 이것은 말투 규칙만큼 중요한 최우선 지침이다.
- 어휘: 국제 통용 표준 교육과정 {LV_RANGE} 범위의 고빈도 일상 어휘만 사용해.
  {MAX_LEVEL + 1}급 이상 수준의 저빈도 한자어·전문 용어·속담·사자성어·어려운 관용구는 쓰지 마.
  꼭 필요한 어려운 단어가 나오면 바로 뒤에 쉬운 말로 짧게 풀어줘.
- 문법: 아래 {LV_RANGE} 문법 목록 안의 문형만 사용해. 목록에 없는 문형은 금지.
[사용 가능 문법 — 초급(1·2급)]
{LEVEL_GRAMMAR_1_2}{_LV_BLOCK_3}{_LV_BLOCK_4}
- 한 문장은 짧게, 한 번에 한 가지 내용만. 학습자가 한 번 듣고 이해할 수 있어야 한다.
{"- ★ 지금은 **초급** 상한이다. 위 목록에 없는 문형은 아무리 자연스러워도 쓰지 마라." if MAX_LEVEL <= 2 else ""}
{"  한 문장을 **한 마디**로 짧게. 접속사로 두 마디를 이어 붙이지 마라." if MAX_LEVEL <= 2 else ""}
- 한 턴을 몇 어절로 할지는 [한 턴 길이]에서 정한다. 여기서 다시 정하지 않는다.
"""

# ★★ v162 — 구어 꼬리말은 등급 목록 **밖**이다. 지우지 않고 **예외로 선언**한다.
#   「-거든」·「-잖아」는 표준 교육과정에서 3급이다. 그런데 입말에서 너무 흔해서
#   빼면 호아랑의 말이 교과서처럼 들린다. 그래서 남긴다 — 다만 **말없이** 남기지
#   않는다. 등급 목록은 「1·2급만」이라 하고 여기서는 3급 문형을 이름 대며 권하면,
#   그것이 곧 두 번째 눈금이다(v161 「지시가 둘이면 목록이 이긴다」와 같은 자리).
#   ★ 예외는 **선언되어 있을 때만** 예외다. 선언되지 않은 예외는 그냥 모순이다.
#   ─ 「-더라고요」(4급)는 초급에서 뺀다. 「그 정도면 괜찮다」의 그 정도 밖이다.
#   ─ 이것은 **알아듣게** 하려는 것이지 학습자에게 만들어 쓰라는 것이 아니다.
_CHUNK_TAIL = ('''"있잖아", "-거든", "-잖아"
    ★ 이 셋은 문법 목록(1·2급) **밖**이다. 그래도 **덩어리로** 쓴다 —
      입말에서 너무 흔해서 빼면 오히려 어색해진다.
...
- 구어 문법을 살려:
  ...
  · 머리말·꼬리말 — {_CHUNK_TAIL}
- 덩어리 표현(구어 관용 표현)을 {LV_BAND} 수준 안에서: "글쎄요", "그러게요", "아직요", "어떡해요", "잠시만요".
- 문어체 접속어(그러나, 따라서, 및, ~하였다)와 딱딱한 설명조는 금지.
- 단, 발음 변이 표기(줄임·경음화 표기)는 쓰지 말고 표준 표기로. 학습자의 말을 일부러 끊지도 마.
"""

# ★ v162 — BASE_PERSONA 는 보통 문자열(중괄호가 들어 있어 f-string 으로 못 만든다)이라
#   자리표를 두고 여기서 갈아 끼운다. 눈금은 위 TURN_LEN_RULE 한 곳뿐이다.
assert "{한턴길이}" in BASE_PERSONA, "BASE_PERSONA 의 [한 턴 길이] 자리표가 사라졌다"
BASE_PERSONA = BASE_PERSONA.replace("{한턴길이}", TURN_LEN_RULE)
...
def build_focus_block(focus: set) -> str:
    """★ v158 — 학습자가 자유 대화 앞에서 고른 「이번에 해 볼 것」.

    ★ 왜 넣나
      4.1.2에서 네 앱을 분석하며 얻은 결론 하나 — 학습 목표와 과제를 걷어 낸
      자유 대화는 범용 생성형 AI와의 음성 대화와 다르지 않다. 남는 교육적
      장치는 미션과 비계와 숙달도 통제뿐이다.
      주제 대화에는 기능 단계라는 목표가 있으나 자유 대화에는 없었다.

    ★ 어떻게 다른가
      미션을 **도구가 주지 않는다.** 학습자가 고른다. 그리고 여기서 하는 일은
      「그것을 하라」고 시키는 것이 아니라 **그것이 필요해지는 자리를 만드는** 것이다.
      시키면 학습자는 시킨 것을 따라 하고, 자리를 만들면 스스로 한다.
    """
    if not focus:
        return ""
    names = [e["name"] for e in IDC_ELEMENTS if e["key"] in focus]
    hows = []
    for e in IDC_ELEMENTS:
        if e["key"] in focus and e.get("elicit"):
            hows.append(f"· {e['name']} — {e['elicit']}")
    return f"""

# ★★ 이번 대화에서 학습자가 해 보겠다고 고른 것 ★★
{" · ".join(names)}

- 학습자가 **스스로 고른** 목표다. 그 자리를 대화 안에서 만들어 줘라.
{chr(10).join(hows)}
- ★ 이름을 입에 올리지 마라. "오늘은 화제 관리를 연습해요" 같은 말은 절대 금지다.
  자리를 만들 뿐, 무엇을 하는 중인지 알려 주지 않는다.
...
- 네 말을 사용자가 못 알아듣는 눈치면, {native}로 아주 짧게 (한 문장 이내) 덧붙여도 좋아. 단, 대화의 기본 언어는 항상 한국어야.
"""
    fusion = """
# 두 페이더의 융합 연산
...
- ★ 이때도 [한 턴 길이]를 지켜라. **예외가 아니다** — 세 마디를 다 짧게 하면
  넉넉히 들어간다. 「우리 지금 반말이지? '어려워.' 다시 말해 볼래?」면 충분하다.
{"  다만 초급이면 ①을 접고 ②③만 붙여도 된다 — 「'학교에서 공부했어.' 다시 말해 볼래?」" if MAX_LEVEL <= 2 else ""}
...
- 한 문장, 짧은 구어체. 국제 통용 표준 교육과정 {LV_CAP} 어휘·문법.
...
3) 각 단계 필드:
   ...
   - expressions: 학습자({my_role or '학습자'} 역할)가 이 단계에서 쓸 만한 자연스러운 한국어 표현 2~3개. 실제 구어체로.
     표현과 cue는 모두 국제 통용 한국어 표준 교육과정 {LV_CAP} 어휘·문법 범위로 작성하라.
...
4) script — 위 기능단계를 처음부터 끝까지 밟아 가는 **모델 대화문** 10~14줄.
   ...
   - 한 줄은 한 문장 또는 짧은 두 문장. 국제 통용 표준 교육과정 {LV_CAP} 어휘·문법으로.
...
- 학습자가 대화를 주도하게 하라. 네 발화 길이는 [한 턴 길이] 그대로다. 네가 먼저 화제를 다 끌고 가지 마라.
...
- [정확성 비교정] 규칙은 상황극 중에도 그대로다. 조사·시제·높임·어휘를 지적하지 마라.
  틀린 데가 있으면 **배역 안에서 바른 꼴로 되말해라**("아, 3층에서 내리시는군요."). 곧장 극으로 복귀.
...
- 국제 통용 한국어 표준 교육과정 **{LV_CAP}** 어휘·문법. 짧은 입말로.
...
2) script — 그 자리에서 오가는 대화 **6~8줄**. 두 사람이 번갈아 말한다.
   ...
   - 두 사람 모두 {lv} 로 말한다. {LV_CAP} 어휘·문법, 실제 입말.
...
async def version_check():
    ...
    try:
        ...
        pass
    return {
        "level": MAX_LEVEL,             # 지금 걸린 숙달도 상한 (실험=3 / 배포=4)
...
async def review_test():
    ...
    prompt = ("너는 따뜻한 한국어 선생님이다. 아래 대화를 한 중급 학습자에게 "
              "100자 안팎으로 '-해요'체 총평을 써라. " + LV_RANGE + " 어휘만 쓴다.\n\n" + demo)
...
async def _handle_session(websocket: WebSocket):
    ...
    # ★ v158 — 학습자가 자유 대화 앞에서 고른 「이번에 해 볼 것」(요소 열쇠, 최대 2개).
    #   4.1.2에서 지적한 자리다 — 목표 없는 자유 대화는 범용 생성형 AI와 다르지 않다.
    #   목표를 **학습자가** 고르게 하고, 그 자리를 상대가 만들어 주게 한다.
    focus_els = {k for k in (websocket.query_params.get("focus", "") or "").split(",")
    ...
    def _fit_intervention() -> str:
        ...
        def _pick(layer: list[list[str]]) -> str:
            ...
            def _rank(qid: str):
                q = next((x for x in QUEST_LLM if x["id"] == qid), None)
                el = q["el"] if q else ""
                # ★ v158 — 학습자가 「이번에 해 볼 것」을 골랐으면 그것을 먼저 준다.
                #   목표를 고르게 해 놓고 그와 상관없는 자리만 열어 주면 고른 뜻이 없다.
                #   다만 **후보를 좁히지는 않는다** — 그 요소의 자리가 아닌데 억지로
                #   끼워 넣으면 v104에서 겪은 「맥락 없이 따라 읽히는」 자리로 돌아간다.
                #   맞는 자리들 가운데 고른 것을 앞세울 뿐이다.
                return (0 if el in focus_els else 1,
                        counts.get(el, 0), cand.index(qid))
...
{extra}
- 국제 통용 표준 교육과정 {LV_CAP} 어휘·문법, 짧은 구어체로.
...
★★ v159 — recasts 칸을 함께 채워라 (대화가 끝난 뒤에만 보여 준다).
  대화 중에 **호아랑이 학습자의 말을 바른 꼴로 되말해 준 자리**를 최대 3개 찾아라.
  · me: 학습자가 실제로 한 말 (그대로, 30자 이내)
  · fix: 호아랑이 되말해 준 **바른 꼴** (30자 이내)
  · part: 둘 사이에서 **달라진 조각 하나** (10자 이내). 예) "에서", "먹었어요"
  ★ 호아랑이 실제로 되말한 자리만 적어라. 없으면 빈 배열로 둔다.
  ★ 학습자가 처음부터 바르게 말한 것은 적지 마라.
  ★ 발음·억양은 다루지 않는다. 글로 드러난 것만 본다.
  예) {{"me":"학교에 공부해요","fix":"학교에서 공부해요","part":"에서"}}
...
JSON만 출력: {{"items":[{{"key":"","grade":"hi|mid|lo","why":""}}],
  ...
  "recasts":[{{"me":"","fix":"","part":""}}]}}"""
        ...
        try:
            ...
            pass
        # ★ v159 — 호아랑이 되말해 준 자리. 이것도 **같은 호출**에서 함께 받았다.
        #   ★ 점수에는 들어가지 않는다. 대화가 끝난 뒤 「아, 여기가 그거였구나」 하고
        #     알아차리게 하는 자리일 뿐이다(noticing). 정확성을 재는 것이 아니다.
        recasts = []
        try:
            for rc in ((data or {}).get("recasts") or [])[:3]:
                if not isinstance(rc, dict):
                    continue
                me = _clean_str(rc.get("me"), 40)
                fix = _clean_str(rc.get("fix"), 40)
                if not me or not fix or me == fix:
                    continue
                part = _clean_str(rc.get("part"), 14)
                # 달라진 조각이 바른 꼴 안에 실제로 있어야 색을 칠할 수 있다
                recasts.append({"me": me, "fix": fix,
                                "part": part if part and part in fix else ""})
        except Exception:
            pass
        return {"items": items, "total": total, "memory": mem,
                "phrases": phrases, "recasts": recasts}
...
총평 글만 출력하라. 제목도 인사말도 붙이지 마라."""
    ...
    async def send_final_score():
        ...
        await websocket.send_text(json.dumps({
            ...
            # ★ v159 — 호아랑이 되말해 준 자리. 점수 밖이다.
            "recasts": idc.get("recasts") or [],
        ...
        async with client.aio.live.connect(model=model_id, config=config) as gemini_session:
            ...
            if rp_plan:
                first_msg = (f"(상황극 시작 — 학습자가 아직 말이 없다) 너는 지금 {rp_plan['place_ko']}의 {rp_plan['ai_role']}(이)야. "
                             ...
                             f"설정된 말투와 페이더에 맞게, {LV_TURN}. 그보다 길게 말하지 마라.")
            else:
