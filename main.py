...
#       Render → Environment → MAX_LEVEL = 3   (실험 기간)
#       변수를 지우거나 4로 두면 = 배포본(1~4급)
#   지금 어느 쪽인지는 /version 의 "level" 로 확인한다.
try:
    MAX_LEVEL = max(2, min(4, int(os.environ.get("MAX_LEVEL", "4"))))
except Exception:
    MAX_LEVEL = 4
LV_CAP   = f"중급({MAX_LEVEL}급 이하)"   # 프롬프트에 넣는 말
