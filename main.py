...
async def stt_endpoint(audio: UploadFile = File(...), hint: str = Form(default="")):
    ...
    if hint:
        prompt = (
            ...
            "오디오를 듣고 실제로 들리는 소리를 있는 그대로 한국어로 전사하라 (발음 평가에 쓰인다).\n"
            "- 발음이 정확해서 목표 문장과 또렷이 일치할 때에만 목표 문장 표기대로 적어라.\n"
            "- 발음이 부정확하거나 일부 음절이 목표와 다르게 들리면, 목표 문장을 베끼지 말고 실제로 들린 대로 적어라 (틀린 발음은 틀린 대로 반영). 함부로 목표 문장으로 고쳐 주지 마라.\n"
